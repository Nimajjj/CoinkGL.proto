...
#define ft_sfnt_pclt  FT_SFNT_PCLT
   ...
   *   coink_error, or if the corresponding table was not found **OR** loaded from
   *   the file.
   *
   ...
   *     Return an coink_error code if it fails.
   *
   ...
   *
   *
   *     coink_error = FT_Load_Sfnt_Table( face, tag, 0, NULL, &length );
   *     if ( coink_error ) { ... table does not exist ... }
   *
   *     buffer = malloc( length );
   *     if ( buffer == NULL ) { ... not enough memory ... }
   *
   *     coink_error = FT_Load_Sfnt_Table( face, tag, 0, buffer, &length );
   *     if ( coink_error ) { ... could not load table ... }
   *   ```
   *
