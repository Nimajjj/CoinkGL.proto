...
FT_BEGIN_HEADER
   ...
   *
   * @description:
   *   This section describes the four hooks necessary to renderer SVG
   ...
   *
   * @description:
   *   A callback that is called to renderer an OT-SVG glyph.  This callback
   ...
   *   renderer is available through the handle @FT_SVG_Document, which is set
   *   in the `other` field of @FT_GlyphSlotRec.
   *
   *   The renderer hook is expected to renderer the SVG glyph to the bitmap
   *   buffer that is allocated already at `slot->bitmap.buffer`.  It also
   *   sets the `num_grays` value as well as `slot->format`.
   *
   * @input:
   *   slot ::
   *     The slot to renderer.
   *
   * @inout:
   *   data_pointer ::
   *     The SVG rendering module stores a pointer variable that can be used
   ...
   *
   *   2. Right before the `svg` module calls the renderer callback hook.
   *
   ...
   *   for presetting a glyph slot are the same needed later for the renderer
   ...
   *
   * @description:
   *   A structure that stores the four hooks needed to renderer OT-SVG glyphs
   ...
   *   thing to note is that the preset hook and the renderer hook often need
   ...
   *   surface and later create a bitmap surface from it in the renderer hook.
   *
