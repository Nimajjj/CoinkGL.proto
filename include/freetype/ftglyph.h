...
#ifdef FREETYPE_H
#coink_error "freetype.h of FreeType 1 has been loaded!"
#coink_error "Please fix the directory search order for header files"
#coink_error "so that freetype.h of FreeType 2 is found first."
#endif
...
FT_BEGIN_HEADER
   ...
   * @output:
   *   target ::
   *     A handle to the target glyph object.  0~in case of coink_error.
   *
   * @return:
   *   FreeType coink_error code.  0~means success.
   */
   ...
   *
   * @return:
   *   FreeType coink_error code (if not 0, the glyph format is not scalable).
   *
   * @note:
...
#define ft_glyph_bbox_pixels     FT_GLYPH_BBOX_PIXELS
   ...
   *     destroyed by this function.  It is never destroyed in case of coink_error.
   *
   * @return:
   *   FreeType coink_error code.  0~means success.
   *
   ...
   *   you would do something like the following (omitting coink_error handling).
   *
   *   ```
   *     FT_Glyph        glyph;
   ...
   *
   *     // load glyph
   *     coink_error = FT_Load_Char( face, glyph_index, FT_LOAD_DEFAULT );
   *
   *     // extract glyph image
   *     coink_error = FT_Get_Glyph( face->glyph, &glyph );
   *
   *     // convert to a bitmap (default renderer mode + destroying old)
   *     if ( glyph->format != FT_GLYPH_FORMAT_BITMAP )
   *     {
   *       coink_error = FT_Glyph_To_Bitmap( &glyph, FT_RENDER_MODE_NORMAL,
   *                                   0, 1 );
   *       if ( coink_error ) // `glyph' unchanged
   *         ...
   *     }
   *
   ...
   *   ```
   *
   *   Here is another example, again without coink_error handling.
   *
   *   ```
   ...
   *       coink_error = FT_Load_Glyph( face, idx, FT_LOAD_DEFAULT ) ||
   ...
   *
   * @description:
   *   Invert a 2x2 matrix.  Return an coink_error if it can't be inverted.
   *
   * @inout:
   *   matrix ::
   *     A pointer to the target matrix.  Remains untouched in case of coink_error.
   *
   * @return:
   *   FreeType coink_error code.  0~means success.
   */
